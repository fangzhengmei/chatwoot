# Chatwoot 邮件渠道完整收发链路分析（v3 - reply_mailer_migration 开关分支分析）

## 修正说明

v2 报告缺少以下分析：
- `reply_mailer_migration` Feature Flag 开关下的发件人与回复地址分支
- 开关打开/关闭时各邮件头的变化对比
- 对入站会话串联命中的实际影响

本报告在 v2 基础上新增第 7 章和第 12 章，详细分析上述内容。
...
## 7. reply_mailer_migration 开关分析（v3 新增）
...
# 辅助方法（来自 ConversationReplyMailer）
...
| `should_use_conversation_email_address?` | 邮箱渠道 或 inbound_email_enabled? |

**简化规则（旧逻辑）**：
- **有自己的发送能力**（OAuth 或 SMTP）→ 用 `@channel.email`
- **无自己的发送能力**（走 Chatwoot 平台 SMTP）→ 用 `@account.support_email` 或 `@inbox.email_address`
...
**简化规则（旧逻辑）**：
- **有 IMAP 抓取能力** → 用 `@channel.email`（客户回复直接发到渠道邮箱，IMAP 抓取）
- **无 IMAP 抓取能力** → 用 `reply+<uuid>@...` 或备用邮箱
...
```
email_channel_type
    ├── :google_oauth           → channel.email
    ├── :microsoft_oauth        → channel.email
    ├── :standard_imap_smtp     → channel.email
    ├── :forwarding_own_smtp    → channel.email
    ├── :imap_chatwoot_smtp
    │       ├── verified_for_sending?   → channel.email
    │       └── !verified_for_sending?  → account_support_email（降级）
    ├── :forwarding_chatwoot_smtp
    │       ├── verified_for_sending?   → channel.email
    │       └── !verified_for_sending?  → account_support_email（降级）
    └── :unknown                 → account_support_email
```

**关键改进（新 vs 旧）**：
1. **更细粒度的分类**：旧逻辑只有「有 OAuth/SMTP」vs「无」，新逻辑细分为 6 种类型
2. **Chatwoot SMTP 场景的 sender 验证**：新增 `verified_for_sending` 检查，未验证时降级到 `account_support_email`（防止邮件被标记为垃圾邮件）
3. **Forwarding 场景的区分**：明确区分「有自己 SMTP」和「无自己 SMTP」
...
| 场景 | 旧逻辑（switch OFF） | 新逻辑（switch ON） |
|------|---------------------|---------------------|
| 邮箱渠道，IMAP 启用 | `@channel.email` | `@channel.email` ✅ 相同 |
| 邮箱渠道，IMAP 禁用 | 需看 `should_use_conversation_email_address?` | `@channel.email` ✅ 明确 |
| 非邮箱渠道 + inbound 开启 | `reply+<uuid>@...`（仅当 should_use...） | `reply+<uuid>@...` ✅ 相同 |
| 非邮箱渠道 + inbound 关闭 | `@inbox.email_address \|\| @agent.email` | `@account.support_email` 🔴 变化 |

**关键变化（非邮箱渠道场景）**：
- **旧逻辑**：回退到 `@inbox.email_address` 或 `@agent.email`
- **新逻辑**：统一回退到 `@account.support_email`
...
| 场景 | IMAP | SMTP | Provider | 旧逻辑（OFF） | 新逻辑（ON） |
|------|------|------|----------|---------------|--------------|
| Google OAuth | ✅/❌ | ✅/❌ | google | `@channel.email` | `@channel.email` ✅ |
| Microsoft OAuth | ✅/❌ | ✅/❌ | microsoft | `@channel.email` | `@channel.email` ✅ |
| 标准 IMAP+SMTP | ✅ | ✅ | 空 | `@channel.email` | `@channel.email` ✅ |
| 仅转发+自己 SMTP | ❌ | ✅ | 空 | `@channel.email` | `@channel.email` ✅ |
| IMAP+Chatwoot SMTP（已验证） | ✅ | ❌ | 空 | `@channel.email` | `@channel.email` ✅ |
| IMAP+Chatwoot SMTP（未验证） | ✅ | ❌ | 空 | `@channel.email` ⚠️ 可能 spam | `@account.support_email` 🔴 安全降级 |
| 仅转发+Chatwoot SMTP（已验证） | ❌ | ❌ | 空 | `@channel.email` | `@channel.email` ✅ |
| 仅转发+Chatwoot SMTP（未验证） | ❌ | ❌ | 空 | `@account.support_email` | `@account.support_email` ✅ |
| 非邮箱渠道 | — | — | — | `@account.support_email` 或 `@inbox.email_address` | `@account.support_email` ✅ 统一 |
...
| 场景 | 旧逻辑（OFF） | 新逻辑（ON） |
|------|---------------|--------------|
| 邮箱渠道（IMAP 启用） | `@channel.email` | `@channel.email` ✅ |
| 邮箱渠道（IMAP 禁用） | 需看 `should_use...` | `@channel.email` ✅ 明确 |
| 非邮箱渠道 + inbound 开启 | `reply+<uuid>@...` | `reply+<uuid>@...` ✅ |
| 非邮箱渠道 + inbound 关闭 | `@inbox.email_address \|\| @agent.email` | `@account.support_email` 🔴 变化 |
...
**重要**：以下邮件头**不受 `reply_mailer_migration` 开关影响**：
...
#### 7.6.1 IMAP 渠道场景（核心邮件渠道）

**IMAP 渠道的会话串联依赖**：
```
IMAP 入站会话查找（两步法）
    ├── 策略一: In-Reply-To → messages.source_id
    └── 策略二: References → messages.source_id 或 fallback 正则
```

**结论**：
- ✅ **完全不受开关影响**
- 原因：In-Reply-To、References、Message-ID 三个关键头均不受开关影响
- From/Reply-To 的变化仅影响客户邮件客户端的「回复按钮」指向

#### 7.6.2 非邮箱渠道场景（API 渠道 + email_continuity）

**非邮箱渠道的会话串联依赖**：
```
ActionMailbox 路由 → ConversationFinder（四策略链）
    ├── 策略一: ReceiverUuidStrategy → 解析 Reply-To 的 reply+<uuid>@...
    ├── 策略二: InReplyToStrategy
    └── 策略三: ReferencesStrategy
```

**开关影响分析**：

| 开关状态 | inbound 开启 | Reply-To 值 | 影响 |
|----------|-------------|-------------|------|
| OFF | ✅ | `reply+<uuid>@...` | ✅ 正常通过策略一命中 |
| ON | ✅ | `reply+<uuid>@...` | ✅ 正常通过策略一命中 |
| OFF | ❌ | `@inbox.email_address` 或 `@agent.email` | ⚠️ 无法通过策略一，依赖策略二/三 |
| ON | ❌ | `@account.support_email` | ⚠️ 无法通过策略一，依赖策略二/三 |

**结论**：
- **inbound 开启时**：开关无影响
- **inbound 关闭时**：旧逻辑和新逻辑都无法通过策略一命中，但新逻辑更统一（统一回退到 `support_email` 而非分散的 inbox/agent 邮箱）

#### 7.6.3 潜在风险场景

**场景 A：IMAP 邮箱，使用 Chatwoot SMTP 且未验证 sender**

| 配置 | 值 |
|------|-----|
| IMAP 启用 | ✅ |
| SMTP 启用 | ❌（走 Chatwoot SMTP） |
| `verified_for_sending` | ❌（未验证） |

**开关关闭时**：
```
From: support@acme.com
```
⚠️ 风险：Chatwoot SMTP 代发 acme.com 的邮件，未验证 sender，容易被标记为垃圾邮件

**开关打开时**：
```
From: accounts@chatwoot.com（假设 support_email 是平台默认值）
```
✅ 安全：使用已验证的平台发件域，但客户看到的发件人是平台地址

**对会话串联的影响**：
- 零影响（In-Reply-To/References/Message-ID 不变）
- 但可能影响邮件到达率（spam 文件夹），进而影响客户能否及时回复

---

**场景 B：非邮箱渠道，inbound 关闭，依赖策略一**

| 配置 | 值 |
|------|-----|
| 渠道类型 | API Channel |
| `inbound_emails` feature | ❌ 关闭 |
| `inbound_email_domain` | 未配置 |

**开关关闭时**：
```
Reply-To: Agent Name <agent@company.com>
```
客户回复邮件时，收件人是 agent 个人邮箱，Chatwoot 可能无法捕获

**开关打开时**：
```
Reply-To: Support <support@company.com>
```
客户回复邮件时，收件人是统一的 support 邮箱，可能更容易被捕获

**对会话串联的影响**：
- 都无法通过策略一（UUID 收件人）命中
- 新逻辑更清晰：明确知道依赖 In-Reply-To/References 策略
- 但实际影响取决于邮件是否能被 Chatwoot 捕获
...
| 维度 | 影响程度 | 说明 |
|------|----------|------|
| **IMAP 渠道会话串联** | ❌ 无影响 | In-Reply-To/References/Message-ID 不受影响 |
| **IMAP 渠道 From 地址** | ✅ 影响（安全改进） | 未验证 sender 时降级到 `support_email`，防止 spam |
| **非邮箱渠道会话串联** | ⚠️ 轻微影响 | inbound 关闭时都无法走策略一，但新逻辑更统一 |
| **邮件到达率** | ✅ 正面影响 | 未验证 sender 时降级，减少 spam 标记 |

**设计意图**：
1. **渐进式迁移**：Feature Flag 允许灰度切换，不影响所有客户
2. **安全性改进**：Chatwoot SMTP 场景强制验证 sender，减少邮件被标记为 spam
3. **逻辑统一**：非邮箱渠道的回退逻辑更明确
4. **向后兼容**：IMAP 渠道的核心串联逻辑（In-Reply-To/References）完全不变
...
└────┬─────┘     └────────┬─────────┘     └──────────┬───────────┘
     ...
     │                    │                          │           │           │    （两者受 reply_mailer_migration 开关影响）
     │                    │                          │           │           │
     │                    │                          │           │           │    Message-ID:
     │                    │                          │           │           │    <conversation/<uuid>/
     │                    │                          │           │           │     messages/<id>@domain>
...
### 10.6 reply_mailer_migration 开关设计

| 维度 | 设计 |
|------|------|
| **作用范围** | 仅影响 `From` 和 `Reply-To` 头 |
| **不影响** | `Message-ID`、`In-Reply-To`、`References`（会话串联核心） |
| **改进点** | Chatwoot SMTP 场景强制 `verified_for_sending`，防止 spam |
| **迁移策略** | Feature Flag 灰度切换，旧逻辑保留为 fallback |
...
**策略一：ReceiverUuidStrategy 说明**：

**场景**：非 IMAP 渠道（如 API Channel + email_continuity），入站邮件走 ActionMailbox 路由。

**原理**：Reply-To 地址格式为 `reply+<conversation-uuid>@<inbound-email-domain>`，客户回复时收件人就是这个地址。
...
**与 reply_mailer_migration 开关的关系**：
- 开关打开/关闭时，只要 `inbound_email_enabled?` 为 true，新老逻辑都生成 `reply+<uuid>@...` 格式
- 只有当 `inbound_email_enabled?` 为 false 时，才会回退到其他地址（此时策略一无法命中）
